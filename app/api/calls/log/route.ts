...
import { logCall, validateCallLogInput } from "@/lib/calls";
...
export async function POST(request: Request) {
  ...
  try {
    const body = await parseJsonBody<Partial<CallLog>>(request);
    ...
    const result = await logCall(body as Partial<CallLog>);
